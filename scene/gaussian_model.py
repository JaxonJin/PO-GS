...
from utils.HGSA import Hierarchical_Granular_Structural_Attention as HGSA
from efficient_kan.kan import kan_opa , kan_cov
...
class GaussianModel:
    ...
    def __init__(self, 
        ...
        # Replace MLP to KAN
        self.mlp_opacity = kan_opa(feat_dim + 3 + self.opacity_dist_dim, feat_dim, n_offsets).cuda()

        self.add_cov_dist = add_cov_dist
        self.cov_dist_dim = 1 if self.add_cov_dist else 0
        # Replace MLP to KAN
        self.mlp_cov = kan_cov(feat_dim+3+self.cov_dist_dim, feat_dim, 7*self.n_offsets).cuda()

        self.color_dist_dim = 1 if self.add_color_dist else 0
        self.mlp_color = nn.Sequential(
            HGSA(feat_dim+3+self.color_dist_dim+self.appearance_dim,7),
            nn.Linear(feat_dim+3+self.color_dist_dim+self.appearance_dim, 3*self.n_offsets),
            nn.Sigmoid()
        ).cuda()
    ...
    def save_mlp_checkpoints(self, path, mode = 'split'):#split or unite
        ...
        if mode == 'split':
            ...
            self.mlp_color.eval()
            # input_tensor = torch.rand(1, self.feat_dim + 3 + self.color_dist_dim + self.appearance_dim).cuda()
            # print(f"Input tensor is on device: {input_tensor.device} with dtype: {input_tensor.dtype}")
