...
def training(dataset, opt, pipe, dataset_name, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, wandb=None, logger=None, ply_path=None):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        # Compute NLPD loss
        nlpd_loss_fn = NLPD().to("cuda")
        nlpd_loss = nlpd_loss_fn(image, gt_image, as_loss=True)
        lambda_nlpd = 0.2

        loss = (((1 - opt.lambda_dssim) * Ll2 + opt.lambda_dssim * ssim_loss) + 0.01*scaling_reg) * (1 - lambda_nlpd) + lambda_nlpd * nlpd_loss
        loss.backward()
        iter_end.record()
        ...
        with torch.no_grad():
            ...
            # Log and save
            training_report(tb_writer, dataset_name, iteration, Ll2, loss, l2_loss, iter_start.elapsed_time(iter_end), testing_iterations, scene, render, (pipe, background), wandb, logger)
...
def training_report(tb_writer, dataset_name, iteration, Ll2, loss, l2_loss, elapsed, testing_iterations, scene : Scene, renderFunc, renderArgs, wandb=None, logger=None):
    if tb_writer:
        tb_writer.add_scalar(f'{dataset_name}/train_loss_patches/l2_loss', Ll2.item(), iteration)
        tb_writer.add_scalar(f'{dataset_name}/train_loss_patches/total_loss', loss.item(), iteration)
        tb_writer.add_scalar(f'{dataset_name}/iter_time', elapsed, iteration)


    if wandb is not None:
        wandb.log({"train_l2_loss":Ll2, 'train_total_loss':loss, })
    ...
    if iteration in testing_iterations:
        ...
        for config in validation_configs:
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                if tb_writer:
                    tb_writer.add_scalar(f'{dataset_name}/'+config['name'] + '/loss_viewpoint - l2_loss', l2_test, iteration)
                    tb_writer.add_scalar(f'{dataset_name}/'+config['name'] + '/loss_viewpoint - psnr', psnr_test, iteration)
                if wandb is not None:
                    wandb.log({f"{config['name']}_loss_viewpoint_l2_loss":l2_test, f"{config['name']}_PSNR":psnr_test})
